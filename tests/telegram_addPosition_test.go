...
func (s *AppTestSuite) TestAddPosition() {
	...
	s.Run("customer's cart is empty", func() {
		...
		err := s.repositories.Customer.Save(ctx, customer)
		require.NoError(err)
		...
		dbCustomer, err := s.repositories.Customer.GetByTelegramID(ctx, telegramID)
		require.NoError(err)

		require.Equal(domain.StateWaitingForOrderType, dbCustomer.TgState)
		// cleanup
		s.repositories.Customer.Delete(ctx, dbCustomer.CustomerID)
	})

	s.Run("customer's cart is not empty", func() {
		customer := domain.NewCustomer(telegramID, username)
		customer.Cart.Add(domain.Position{
